...
public class Board
{
    ...
    {
        Vector3 origin = new Vector3(-boardSizeX * 0.5f + 0.5f, -boardSizeY * 0.5f + 0.5f, -4.5f);
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                go.transform.position = origin + new Vector3(x, y, 3f);
                go.transform.rotation = new Quaternion(0f, 0, 0, 0);
    ...
    public void Swap(Cell cell1, Cell cell2, Action callback)
    {
        ...
        item2.View.DOMove(cell1.transform.position, 0.2f).OnComplete(() => { if (callback != null) callback(); });
    }
